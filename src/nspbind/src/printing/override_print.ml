...
(* $Id: override_print.ml,v 1.6 2019-05-21 11:14:31 jpc Exp $ *)
...
let translate_file ppf dst_efname =
  let basename = Filename.basename
      (Configuration.get_overrides_source_file ()) in
  let override_for_c =
    Configuration.get_path_to_override_for_c () ^ basename in
  let override_for_h =
    Configuration.get_path_to_override_for_h () ^ basename in
  File.set_filenames dst_efname override_for_c override_for_h;
