...
def find_enum_defs(buf, enums=[]):
    ...
    while pos < len(buf):
        ...
        #sys.stderr.write('Try to enum match:|%s|\n' % buf)
        if not m: break
...
func_new_pat_extended = re.compile('(\w+)_new_(\w+)$')
...
class DefsWriter:
        ...
        for cname, isflags, entries in enums:
            ...
            if self.namespace:
                ...
                if m:
                    module = m.group(1)
                    name = m.group(2)
            #jpc: name -> cname in define-flags and define-enum
            if isflags:
                fp.write('(define-flags ' + cname + '\n')
            else:
                fp.write('(define-enum ' + cname + '\n')
            if module:
        ...
        for klass, parent in objdefs:
            ...
            if self.namespace:
                ...
                if m:
                    cmodule = m.group(1)
                    cname = m.group(2)
            # jpc: name of object is klass instead of cname
            #fp.write('# cname:' + cname + ' klass:' + klass + '\n')
            fp.write('(define-object ' + klass + '\n')
            if cmodule:
    ...
    def _define_func(self, buf):
        ...
        for p in buf:
            if not p:
                continue
            # jpc 2015 add the possibility to keep the
            # deprecated or available macros of GTK
            # sys.stderr.write('Try to tag match:|%s|\n' % p)
            m = proto_tag_pat.match(p)
            if m == None:
                #sys.stderr.write('Try to match:|%s|\n' % p)
                m = proto_pat.match(p)
                if m == None:
                    if self.verbose:
    ...
    def _write_func(self, name, ret, args, available):

        # try to detect if its a constructor
        ...
        # jpc: extended to other patterns
        m = func_new_pat_extended.match(name)
        if pointer_pat.match(ret) and m:
            cname = ''
            for s in m.group(1).split ('_'):
                cname += s.title()
        ...
        # decide for a method or a function
        # note that gtk_application_window_new is ambiguous
        # method of gtk_application or constructor of gtk_application_window
        # if its a constructor pattern we prefer constructor to method

        if constructor == False and len(args) >= 1:
            ...
            if m:
                ...
                if munged_name[:len(obj)] == obj.lower():
                    self._write_method(obj, name, ret, args, available)
                    return
        ...
        if constructor :
            # jpc
            # prefered return type for constructors
            self.fp.write('  (is-constructor-of "' + cname + '")\n')
            self._write_return(cname + '*')
        else:
    ...
    def _write_method(self, obj, name, ret, args, available):
        ...
        self.fp.write('  (c-name "' + name + '")\n')
        if available != 'GDK_AVAILABLE_IN_ALL' and available != 'G_AVAILABLE_IN_ALL' and  available != 'GLIB_AVAILABLE_IN_ALL' and  available != 'GI_AVAILABLE_IN_ALL':
            self.fp.write('  (available "' + available + '")\n')
        self._write_return(ret)
